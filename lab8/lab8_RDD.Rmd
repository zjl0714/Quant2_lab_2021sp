...
$$
\begin{aligned}
& \mu^{(1)}_{+}(0) + (\mathbf{R}'\mathbf{W}_{+}\mathbf{R})^{-1}\left(\mathbf{R}'\mathbf{W}_{+}\mathbf{S}_2\frac{\mu^{(2)}_{+}(0)}{2}\right) + (\mathbf{R}'\mathbf{W}_{+}\mathbf{R})^{-1}\left(\mathbf{R}'\mathbf{W}_{+}\nu\right) \\
\end{aligned}
$$
\pause
...
\pause
- Intuitively, we use another local regression estimator with bandwidth $b_N$ to estimate the first-order derivative of $\mu_{+}$ and $\mu_{-}$ and subtract them from $\hat \mu_{+}$ and $\hat \mu_{-}$.
\pause
